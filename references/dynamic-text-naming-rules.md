...
不得使用 `small-ranking`、`ranking-18px`、`ranking-v2` 等视觉值、版本号或临时描述作为复合业务域。若两个不同 HTML 无法获得稳定场景限定，使用 `confirm`，不得写回名称或上传。
...
完整预览图与切图的文字差异只能用于候选发现，不得用于推断业务域或字段语义。颜色、字号、字重、位置和尺寸不得用于推断业务域或字段语义。重复文案生成后的完整 HTML 只用于决定同一业务字段能否复用名称；HTML 不同表示需要稳定场景限定，但具体限定仍须来自业务上下文。
...
对确认属于同一业务字段的重复文案读取 styled text segments，并在命名阶段生成规范 HTML。按“逐字相同原文 + 同一业务字段 + 完整 HTML”一次确定最终名称：三项均相同的节点复用完全相同的名称；HTML 不同的组使用“稳定场景-业务域”复合业务域生成不同名称。style 属性固定按颜色、字号、字重、行高输出；比较范围包括文案、颜色、字号、字重、行高、换行、空白和 styled text 分段生成的全部 HTML，不得只比较字号。该流程无论是否上传 Page Center 都必须执行。
...
用户要求上传 Page Center 时，名称同时是唯一的 HTML key。直接使用命名阶段已经确定的最终名称和 HTML 建立完整 `key -> HTML` 映射；该步骤只做安全校验，不再分组或改名。只要同一 key 出现两个不同 HTML，就在任何 Figma 或 Page Center 写入前停止，把冲突项标记为 `confirm`；不得先写回名称或上传无冲突项，也不得用后一个值覆盖前一个值。
...
重复文案合并预览时必须显示其逐字相同的 `characters` 原文；同一业务字段存在不同原文时仍逐项列出，不得用“奖励名称”“当前轮次”“徽章解锁提示”等中文业务标签代替原文。
