...
只根据文本节点的 `characters` 判断，不根据 `node.name`、样式或可见性判断：
...
当前名称看似合规的候选仍须进入完整复核，不能因正则通过而跳过。非候选使用 `skip`。
...
`business-domain` 表示该一级板块承载的稳定业务对象或功能模块。必须结合板块内标题、原文、相邻标签、组件含义和平行字段确定，并且只能使用一个小写英文单词，例如：
...
相同业务字段复用完全相同的完整名称。只有业务含义确实不同才创建不同名称。视觉样式、页面位置、节点顺序、显隐状态、语言版本和一级板块编号本身都不是拆分名称的理由。
...
- `skip`：`characters` 不匹配候选正则；
- `confirm`：是候选，但业务域、字段语义、缩短方式或冲突处理缺少可靠证据。
